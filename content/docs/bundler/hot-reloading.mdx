...
<Callout type="info">HMR is enabled by default when using Bun's full-stack development server.</Callout>
...
<Callout type="warning">This callback is not called on route navigation or when the browser tab closes.</Callout>
...
<Callout type="info">For compatibility with Vite, the above events are also available via `vite:*` prefix instead of `bun:*`.</Callout>
