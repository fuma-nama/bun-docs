...
<Frame caption="Left: Bun v1.1.44. Right: Node.js v23.6.0">
  <img src="https://bun.com/docs/images/bun-s3-node.gif" alt="Bun's S3 API is fast" />
</Frame>
...
const r2 = new S3Client({
  ...
  secretAccessKey: "secret-key",
  bucket: "my-bucket",
  endpoint: "https://<account-id>.r2.cloudflarestorage.com", // [!code highlight]
});
```
...
const spaces = new S3Client({
  ...
  bucket: "my-bucket",
  // region: "nyc3",
  endpoint: "https://<region>.digitaloceanspaces.com", // [!code highlight]
});
```
...
const minio = new S3Client({
  ...
  endpoint: "http://localhost:9000", // [!code highlight]
});
```
...
const supabase = new S3Client({ // [!code highlight]
  accessKeyId: "access-key",
  secretAccessKey: "secret-key",
  bucket: "my-bucket",
  region: "us-west-1",
...
const s3WithEndpoint = new S3Client({
  accessKeyId: "access-key",
  secretAccessKey: "secret-key",
  endpoint: "https://<bucket-name>.s3.<region>.amazonaws.com", // [!code highlight]
  virtualHostedStyle: true, // [!code ++]
});
...
const r2WithEndpoint = new S3Client({
  accessKeyId: "access-key",
  secretAccessKey: "secret-key",
  endpoint: "https://<bucket-name>.<account-id>.r2.cloudflarestorage.com", // [!code highlight]
  virtualHostedStyle: true, // [!code ++]
});
```
...
const client = new S3Client({ // [!code highlight]
  accessKeyId: "your-access-key",
  secretAccessKey: "your-secret-key",
  bucket: "my-bucket",
  // sessionToken: "..."
...
await client.write("my-file.txt", "Hello World!"); // [!code highlight]
await client.write("my-file.txt", new Response("Hello World!")); // [!code highlight]
...
await client.delete("my-file.txt"); // [!code highlight]
// equivalent to
...
const exists = await client.exists("my-file.txt"); // [!code highlight]
// equivalent to
...
```ts title="s3.ts"
const partial = s3file.slice(0, 1024); // [!code highlight]
...
```ts title="s3.ts"
await s3file.delete(); // [!code highlight]
// await s3File.unlink();
```
...
// Write string
await S3Client.write("my-file.txt", "Hello World"); // [!code highlight]

// Write JSON with type
await S3Client.write("data.json", JSON.stringify({ hello: "world" }), { // [!code highlight]
  ...credentials,
  type: "application/json",
});
...
await S3Client.write("data.bin", res, credentials); // [!code highlight]

// Write with ACL
await S3Client.write("public.html", html, { // [!code highlight]
  ...credentials,
  acl: "public-read",
  type: "text/html",
});
...
const url = S3Client.presign("my-file.txt", { // [!code highlight]
  ...credentials,
  expiresIn: 3600,
});
```
...
const allObjects = await S3Client.list(null, credentials); // [!code highlight]

// List (up to) 500 objects under `uploads/` prefix, with owner field for each object
const uploads = await S3Client.list({ // [!code highlight]
  prefix: 'uploads/',
  maxKeys: 500,
  fetchOwner: true,
}, credentials);
...
if (uploads.isTruncated) {
  ...
  const moreUploads = await S3Client.list({ // [!code highlight]
    prefix: 'uploads/',
    maxKeys: 500,
...
const exists = await S3Client.exists("my-file.txt", credentials); // [!code highlight]
```
...
const bytes = await S3Client.size("my-file.txt", credentials); // [!code highlight]
```
...
await S3Client.delete("my-file.txt", credentials); // [!code highlight]
// equivalent to
// await new S3Client(credentials).delete("my-file.txt");

// S3Client.unlink is alias of S3Client.delete
await S3Client.unlink("my-file.txt", credentials); // [!code highlight]
```
