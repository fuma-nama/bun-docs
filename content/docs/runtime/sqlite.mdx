...
```ts title="db.ts"
import { Database } from "bun:sqlite";
const db = new Database("mydb.sqlite", { readonly: true }); // [!code highlight]
```
...
```ts title="db.ts"
import { Database } from "bun:sqlite";
const db = new Database("mydb.sqlite", { create: true }); // [!code highlight]
```
...
const strict = new Database(":memory:", { strict: true }); // [!code highlight]
...
```ts title="db.ts"
import db from "./mydb.sqlite" with { type: "sqlite" }; // [!code highlight]
...
{
  using db = new Database("mydb.sqlite"); // [!code highlight]
  using query = db.query("select 'Hello world' as message;"); // [!code highlight]
...
const contents = olddb.serialize(); // => Uint8Array // [!code highlight]
...
query.all({ $message: "Hello world" }); // [!code highlight]
```
...
```ts title="db.ts"
const query = db.query(`select $message;`);
query.get({ $message: "Hello world" }); // [!code highlight]
// => { $message: "Hello world" }
```
...
const query = db.query("SELECT title, year FROM movies").as(Movie); // [!code highlight]
...
for (const row of query.iterate()) { // [!code highlight]
  console.log(row);
}
```
...
```ts title="db.ts"
const query = db.query("SELECT * FROM foo");
for (const row of query) { // [!code highlight]
  console.log(row);
}
```
...
query.values({ $message: "Hello world" }); // [!code highlight]
query.values(2); // [!code highlight]
// [
//   [ "Iron Man", 2008 ],
//   [ "The Avengers", 2012 ],
...
console.log(query.toString()); // => "SELECT NULL" // [!code highlight]

query.run(42);
console.log(query.toString()); // => "SELECT 42" // [!code highlight]

query.run(365);
console.log(query.toString()); // => "SELECT 365" // [!code highlight]
```
...
const db = new Database(":memory:", { safeIntegers: true }); // [!code highlight]
...
const db = new Database(":memory:", { safeIntegers: false }); // [!code highlight]
const query = db.query(`SELECT ${BigInt(Number.MAX_SAFE_INTEGER) + 102n} as max_int`);
const result = query.get();
...
const insertCats = db.transaction(cats => { // [!code highlight]
...
const insertCats = db.transaction(cats => {
  for (const cat of cats) insert.run(cat); // [!code highlight]
  return cats.length;
});
...
const db = new Database();
db.loadExtension("myext"); // [!code highlight]
```
...
Database.setCustomSQLite("/path/to/libsqlite.dylib"); // [!code highlight]
...
db.fileControl(constants.SQLITE_FCNTL_PERSIST_WAL, 0); // [!code highlight]
```
